...
const ImageCompareBlock: React.FC<Props> = ({ index, styles, imageCompareData }) => {
  ...
  const beforeSrc =
    imageCompareData.beforeImage?.src ? withBasePath(imageCompareData.beforeImage.src) : "";
  const afterSrc =
    imageCompareData.afterImage?.src ? withBasePath(imageCompareData.afterImage.src) : "";

  // Compare side-by-side (individual)
  if (imageCompareData.type === "individual" && beforeSrc && afterSrc) {
    return (
      <div key={index} className={baseClasses}>
        <div className={`flex gap-4 justify-center ${containerAlignment}`} style={{ width }}>
          {[imageCompareData.beforeImage, imageCompareData.afterImage].map((img, i) => {
        ...
        <div className={containerAlignment} style={{ width }}>
          <Suspense fallback={<div className="h-64 bg-gray-200 animate-pulse rounded" />}>
            <CompareImage
              ...
              onSliderPositionChange={(pos: number) => setSliderPercentage(Math.round(pos * 100))}
