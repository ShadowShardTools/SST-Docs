    ...
    <div className="flex flex-col gap-1">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div
            ref={dialogRef}
            className={`w-[min(760px,92vw)] max-h-[80vh] overflow-hidden rounded-lg border ${styles.sections.siteBorders} ${styles.sections.contentBackground}`}
          >
            <div
              className={`flex items-center justify-between px-4 py-2 border-b ${styles.sections.siteBorders}`}
            >
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className={`inline-flex items-center gap-1 px-2 py-1 ${styles.buttons.common}`}
                  onClick={goUp}
                  disabled={currentDir === "."}
                  aria-label="Go up"
                >
              ...
              {!loading && !browseError && (
                <div className="max-h-[55vh] overflow-y-auto space-y-1">
                  ...
                  {sortedEntries.files
                    .filter(
                      (entry) => isAllowedFile(entry.name) && isInRequiredFolder,
                    )
                    .map((entry) => (
                      <button
                        key={`file-${entry.name}`}
                  ...
                  {!sortedEntries.dirs.length &&
                    !sortedEntries.files.filter((entry) =>
